    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->ganttView->leftView(), SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(showContextMenu(QPoint)));
    ...
    connect(ui->ganttView->graphicsView(), SIGNAL(clicked(QModelIndex)), this, SLOT(slotClicked(QModelIndex)));
    connect(ui->ganttView->graphicsView(), SIGNAL(qrealClicked(QModelIndex)), this, SLOT(slotDoubleClicked(QModelIndex)));
}
...
void MainWindow::enableActions(const QItemSelection &selected)
{
    ...
    if (model->data(model->index(selectedIndex.row(), 1)) == KGantt::TypeEvent || model->data(model->index(selectedIndex.row(), 1)) == KGantt::TypeTask) {
...
void MainWindow::addDemoEntry()
{
    ...
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << codeFreeze << new MyStandardItem(KGantt::TypeEvent) << new MyStandardItem(now, KGantt::StartTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << packaging << new MyStandardItem(KGantt::TypeTask) << new MyStandardItem(now.addDays(5), KGantt::StartTimeRole)
                               << new MyStandardItem(now.addDays(10), KGantt::EndTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>() << upload << new MyStandardItem(KGantt::TypeTask)
    ...
    softwareRelease->appendRow(QList<QStandardItem *>()
                               << testing << new MyStandardItem(KGantt::TypeTask) << new MyStandardItem(now.addSecs(3 * 60 * 60), KGantt::StartTimeRole)
                               << new MyStandardItem(now.addDays(5), KGantt::EndTimeRole));
    softwareRelease->appendRow(QList<QStandardItem *>() << updateDocumentation << new MyStandardItem(KGantt::TypeTask)
                                                        << new MyStandardItem(now.addSecs(3 * 60 * 60), KGantt::StartTimeRole)
                                                        << new MyStandardItem(now.addDays(3), KGantt::EndTimeRole));

    model->appendRow(QList<QStandardItem *>() << softwareRelease << new MyStandardItem(KGantt::TypeSummary));
